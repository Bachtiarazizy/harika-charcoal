...
import { Mountain, ChevronRight, Filter, Grid, List, Search, Download, ArrowRight } from "lucide-react";
            ...
            <div className={`space-y-3 ${viewMode === "list" ? "flex-1" : ""}`}>
              ...
              {viewMode === "list" && (
                <div className="flex items-center gap-2 text-primary hover:text-primary/80 transition-colors">
                  <span className="body-sm font-medium">View Details</span>
      ...
      <section className="py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-8 lg:mb-12">
              ...
              >
                ...
                {categories.map((category) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${selectedCategory === category.slug.current ? "bg-primary text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                  >
                    {category.name}
                  </button>
                ))}
            ...
            {/* Results Count */}
            <div className="text-sm text-secondary">{loading ? "Loading..." : `${filteredProducts.length} products found`}</div>
          </motion.div>
          ...
          <div className={`gap-6 mb-8 lg:mb-12 ${viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : "flex flex-col space-y-6"}`}>
            {error ? (
              ...
              // Products list
              filteredProducts.map((product, index) => <ProductCard key={product._id} product={product} index={index} />)
            ) : (
              // Empty state
              <div className="col-span-full text-center py-12">
